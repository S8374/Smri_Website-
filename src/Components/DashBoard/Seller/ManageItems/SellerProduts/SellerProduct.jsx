...
export default function SellerProduct() {
    ...
    return (
        <div className="p-4">
            <h2 className="text-2xl font-bold mb-4 text-center">Your Listed Products</h2>
                        ...
                        {sellerProducts.length > 0 ? (
                            sellerProducts.map((product) => (
                                <tr key={product._id} className="border hover:bg-gray-50 transition">
                                    ...
                                    <td className="border p-3 font-semibold">{product.title}</td>
                                    <td className="border p-3">{product.stock}</td>
                                    <td className="border p-3 text-blue-600 font-medium">${product.price}</td>
                                    ...
                                    <td className="border p-3">
                                        <button onClick={() => handleUpdate(product)} className="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">Edit</button>
                                        <button onClick={() => handleDelete(product)} className="ml-2 px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600">Delete</button>
                                    </td>
                                </tr>
                            ))
                        ) : (
                            <tr>
                                <td colSpan="7" className="text-center p-5 text-gray-500">No products found</td>
                            </tr>
                        )}
                    </tbody>
                </table>
                ...
                {sellerProducts.length > 0 ? (
                    sellerProducts.map((product) => (
                        <div key={product._id} className="bg-white shadow-md rounded-lg p-4">
                            ...
                            <p className="text-green-600">-{product.discount_percent}% (${product.discount_price})</p>
                            <p className="text-sm text-gray-600">Category: {product.category}</p>
                            <div className="mt-3 flex gap-2">
                                <button onClick={() => handleUpdate(product)} className="w-full py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Edit</button>
                                <button onClick={() => handleDelete(product)} className="w-full py-2 bg-red-500 text-white rounded hover:bg-red-600">Delete</button>
                            </div>
                        </div>
                    ))
                ) : (
